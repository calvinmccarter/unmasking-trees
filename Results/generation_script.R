...
data = read.csv("Results_tabular - generation.csv2")
...
F1_vars = c('iris', 'wine', 'parkinsons', 'climate_model_crashes', 'connectionist_bench_sonar', 'ionosphere', 'qsar_biodegradation', 'seeds', 'glass', 'ecoli', 'yeast', 'libras', 'planning_relax', 'blood_transfusion', 'breast_cancer_diagnostic', 'connectionist_bench_vowel', 'bean', 'car','congress','tictactoe')

methods = c('GaussianCopula', 'TVAE', 'CTGAN', 'CTABGAN', 'stasy', 'TabDDPM', 'forest_diffusion_vp_nt51_ycond', 'forest_diffusion_flow_nt51_ycond', 'utrees')
methods_all = c('GaussianCopula', 'TVAE', 'CTGAN', 'CTABGAN', 'stasy', 'TabDDPM', 'forest_diffusion_vp_nt51_ycond', 'forest_diffusion_flow_nt51_ycond', 'utrees', 'oracle')

data_summary = data.frame(matrix(ncol = 10, nrow = 10))
...
data_summary_rank = data.frame(matrix(ncol = 9, nrow = 10))
colnames(data_summary_rank) = methods
rownames(data_summary) = c("W_train", "W_test", "coverage_train", "coverage_test", "R2_fake", "F1_fake", "class_score", "percent_bias", "coverage_rate", "time")

W_train = data.frame(matrix(ncol = 9, nrow = length(W_vars)))
colnames(W_train) = methods
rownames(W_train) = W_vars
W_test = data.frame(matrix(ncol = 9, nrow = length(W_vars)))
...
R2_fake = data.frame(matrix(ncol = 9, nrow = length(R2_vars)))
colnames(R2_fake) = methods
rownames(R2_fake) = R2_vars
F1_fake = data.frame(matrix(ncol = 9, nrow = length(F1_vars)))
colnames(F1_fake) = methods
rownames(F1_fake) = F1_vars
percent_bias = data.frame(matrix(ncol = 9, nrow = length(R2_vars)))
colnames(percent_bias) = methods
rownames(percent_bias) = R2_vars
coverage_rate = data.frame(matrix(ncol = 9, nrow = length(R2_vars)))
...
rownames(data_summary_t) = c("GaussianCopula", "TVAE", "CTGAN", "CTAB-GAN+", "STaSy", "TabDDPM", "Forest-VP", "Forest-Flow", "UTrees", "Oracle")
rownames(data_summary_rank_t) = c("GaussianCopula", "TVAE", "CTGAN", "CTAB-GAN+", "STaSy", "TabDDPM", "Forest-VP", "Forest-Flow", "UTrees")
...
data_x = read.csv('C:/Users/Alexia-Mini/Downloads/Results_tabular - ablation_iris.csv')
xtable(data_x, type = "latex")
...
# For building the barplot, to csv
write.csv(W_test, file="gen_W.csv")
write.csv(coverage_test, file="gen_cov.csv")
write.csv(R2_fake, file="gen_R2.csv")
write.csv(F1_fake, file="gen_F1.csv")
write.csv(ClassScore, file="gen_disc.csv")
write.csv(percent_bias, file="gen_pb.csv")
write.csv(coverage_rate, file="gen_cr.csv")
}
