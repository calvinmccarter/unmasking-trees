#!/usr/bin/env python
# coding: utf-8
# Adapted from https://github.com/SamsungSAILMontreal/ForestDiffusion/blob/main/script_generation.py
...
parser.add_argument("--methods", type=str, nargs='+', default=['utrees'], help="oracle, CTGAN, GaussianCopula, TVAE, CopulaGAN, CTABGAN, stasy, TabDDPM, forest_diffusion")
        ...
        for method_index in range(method_index_start, len(args.methods)):
            ...
            for n in range(args.nexp):
                ...
                if method == 'oracle':
                    ...
                    for i in range(Xy_train_used.shape[1]):
                    ...
                    utree.fit(X=Xy_train_used, quantize_cols=quantize_cols)
                    Xy_fake = utree.generate(n_generate=args.ngen*Xy_train_used.shape[0])
                    Xy_fake = Xy_fake.reshape(args.ngen, Xy_train_used.shape[0], Xy_train_used.shape[1])
